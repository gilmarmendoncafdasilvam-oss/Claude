...
export default function MemberReportsPage() {
  ...
  if (!can("reports.view")) return <AccessDenied message="Você não tem permissão para visualizar relatórios." />
                ...
                >
                  ...
                  <div className="col-span-2 flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                    <Button variant="ghost" size="sm" asChild>
                      <Link href={`/member/reports/${report.id}`}><Eye className="h-3.5 w-3.5" /></Link>
                    </Button>
                    {can("reports.edit") && (
                      <Button variant="ghost" size="sm" asChild>
                        <Link href={`/member/reports/${report.id}/edit`}><Edit className="h-3.5 w-3.5" /></Link>
                      </Button>
                    )}
                    <Button variant="ghost" size="sm" asChild>
                      <Link href={`/member/reports/${report.id}/print`} target="_blank" title="Exportar PDF">
